...
{
	tmp.pdf <- paste("temp.report", round( runif(1, 1, 1000) ), "pdf", sep=".");
...
Report.title<-function( doc.title, page.header, page.footer )
{
	...
	g_rpt$doc.title   <<- doc.title;
	g_rpt$page.header <<- page.header;
	g_rpt$page.footer <<- page.footer;
}
...
Report.AddHeadline<-function( str, level=1, options=list() )
{
	...
	g_rpt$matr.num <<- g_rpt$matr.num+1;
	g_rpt$matr[[ g_rpt$matr.num ]] <<- list(id="H", str=str, level=level, heading.no=get_headingno(level),options=options);
}


get_headingno<-function(level)
{
	if ( level == length(g_rpt$heading.no)+1 )
	{
		g_rpt$heading.no <<- c( g_rpt$heading.no, 1 )
		return(paste(g_rpt$heading.no, sep="",collapse="."));
	}

	if ( level <= length(g_rpt$heading.no) )
	{
		g_rpt$heading.no[level] <<- g_rpt$heading.no[level]+1;
		if (level<length(g_rpt$heading.no))
			for(i in length(g_rpt$heading.no):(level+1))
				g_rpt$heading.no <<- g_rpt$heading.no[-i];
...
Report.AddParaLine<-function( str, tab.pos=c(), tab.align=c(), options=list(), offset=0 )
{
	...
	g_rpt$matr.num <<- g_rpt$matr.num+1;
	g_rpt$matr[[ g_rpt$matr.num ]] <<- list(id="P", str=str, tab.pos=tab.pos, tab.align=tab.align, offset=offset);
}
...
Report.AddTable<-function( df, title=NULL, frame=c("full","wang", "san"), col.names=NULL, col.width=NULL, col.align=NULL, offset.x =NULL, max.show=0, options=list() )
{
	...
	g_rpt$matr.num <<- g_rpt$matr.num+1;
	g_rpt$matr[[ g_rpt$matr.num ]] <<- list(id="TB", data=df, 
				title = title, 
				frame = frame,
				col.names=col.names, 
				col.width=col.width, 
...
Report.AddFigure<-function( fCallDraw, title, size, left.margin=0, options=list(border=0) )
{
	...
	g_rpt$matr.num <<- g_rpt$matr.num+1;
	g_rpt$matr[[ g_rpt$matr.num ]] <<- list(id="F", title=title, size=size, fCallDraw=fCallDraw, fCallDraw2=NULL, left.margin=left.margin);
}
...
Report.AddPageBreak<-function()
{
	...
	g_rpt$matr.num <<- g_rpt$matr.num+1;
	g_rpt$matr[[ g_rpt$matr.num ]] <<- list(id="NEWPAGE");
}
...
Report.AddParaBreak<-function(height=48)
{
	...
	g_rpt$matr.num <<- g_rpt$matr.num+1;
	g_rpt$matr[[ g_rpt$matr.num ]] <<- list(id="NEWLINE", height=height);
}
...
Report.Output<-function( file )
{
	...
	ret <- FALSE;
	pdf( g_rpt$tmpfile, paper="letter", width=7, height=9.5);
...
Report.do_pagebody<-function( page_no )
{
	if (.RR("debug", 0))
		cat("PageNo:", page_no, "Width,Height", 
			length(g_rpt$pages[[page_no]]$LayoutWidth),
			length(g_rpt$pages[[page_no]]$LayoutHeight), "\n" );

	cat("The page", page_no, " is being plotted.............\n");

	nLayRange <- g_rpt$pages[[page_no]]$LayoutRange;
	nf <- layout( g_rpt$pages[[page_no]]$LayoutMatrix, 
				 widths =g_rpt$pages[[page_no]]$LayoutWidth, 
				 heights=g_rpt$pages[[page_no]]$LayoutHeight, TRUE);
	Report.do_pageheader(page_no);		

	for(i in nLayRange[1]:nLayRange[2])
	{
		if ( g_rpt$layout[[i]]$id=="H" )					
			Report.do_heading( g_rpt$layout[[i]] );		
	 
		if ( g_rpt$layout[[i]]$id=="P" )					
			Report.do_paragraph( g_rpt$layout[[i]] );		
		 
		if ( g_rpt$layout[[i]]$id=="TH" )					
			Report.do_th( g_rpt$layout[[i]] );		

		if ( g_rpt$layout[[i]]$id=="TR" )					
			Report.do_tr( g_rpt$layout[[i]] );		
	
		if ( g_rpt$layout[[i]]$id=="F")					
			Report.do_figures( g_rpt$layout[[i]] );		

		if ( g_rpt$layout[[i]]$id=="BR" )					
			Report.do_br( g_rpt$layout[[i]] );		

		if ( g_rpt$layout[[i]]$id=="BRECT" )					
			Report.do_brect( g_rpt$layout[[i]] );		
	}
...
Report.do_pagecover<-function()
{
	...
	text(7*254/2, 1600, g_rpt$doc.title, cex=2, adj=0.5);
	text(700, 1200, paste("Data File: ",  g_rpt$dat.file,  sep=""), cex=0.8, adj=0);
...
Report.do_pageheader<-function( pageno )
{
	...
	text(20, 20, g_rpt$page.header, cex=0.8, adj=0);
...
Report.do_pagefooter<-function()
{
	...
	text(7*254-50, 30, g_rpt$page.footer, cex=0.8, adj=1);
}
...
{
	par(mar=c(0,0,0,0))
	plot(NA, NA, type="l", xlim=c(0, g_rpt$page.width), ylim=c(0, lay$size[2]), xlab="", ylab="", lwd=2, xaxt="n", yaxt="n", bty="n");

	text(0 + g_rpt$page.offset, lay$size[2]/4, lay$str, cex=lay$cex, adj = c(0, 0) );
}

Report.do_paragraph<-function( lay )
{
	par(mar=c(0,0,0,0))
	plot(1:20, 1:20, type="n", xlim=c(0, g_rpt$page.width), ylim=c(0, lay$size[2]), xlab="", ylab="", lwd=2, xaxt="n", yaxt="n", bty="n");
	...
	for (i in 1:length(lay$sublay))
	{
		...
		if (sub$tab.align=="R")
		{
			...
			if (is.null(sub.next))
				pos.x <- g_rpt$page.width
			else				
				pos.x <- sub.next$tab.pos[1];
		}

		text(pos.x+ g_rpt$page.offset+lay$offset, lay$size[2]/4, sub$str, cex=1, adj = adj, vfont=c("serif", "plain")  );
	}
}
...
{
	par(mar=c(0,0,0,0))
	plot(NA, NA, type="n", xlim=c(0, g_rpt$page.width), ylim=c(0, lay$size[2]), xlab="", ylab="", lwd=2, xaxt="n", yaxt="n", bty="n");
	...
	for (i in 1:length(lay$col.names))
	{
		...
		if (lay$frame=="full")
		{
			rect(tab.pos[i]  +g_rpt$page.offset+layoff, -1, 
				 tab.pos[i+1]+g_rpt$page.offset+layoff, lay$size[2]+1, col=NA, border="black");
		}

		if (lay$frame=="wang")
		{
			x0 <- tab.pos[i]  +g_rpt$page.offset+layoff;
			x1 <- tab.pos[i+1]+g_rpt$page.offset+layoff;
			y0 <- -1;
			y1 <- lay$size[2]+1;
		...
		text(pos.x+ g_rpt$page.offset + layoff, lay$size[2]/4, labels=lay$col.names[i], cex=1, adj = adj, vfont=c("serif", "plain")  );
	}
}
...
Report.do_tr<-function( lay )
{
	...
	for (i in 1:length(lay$data))
	{
		...
		if (lay$frame=="full")
		{
			rect(tab.pos[i]  +g_rpt$page.offset+layoff, 0-1, 
				 tab.pos[i+1]+g_rpt$page.offset+layoff, lay$size[2]+1, col=NA, border="black");
		}
		...
		text(pos.x+g_rpt$page.offset+layoff, lay$size[2]/4, labels=as.character(unlist(lay$data[i])), cex=0.9, adj = adj, vfont=c("serif", "plain")  );
	}
}
...
{
	g_rpt$page.width  <<- 7*254;
	g_rpt$page.height <<- 9.5*254;
	...
	nlay <-1;
	for(i in 1:length(g_rpt$matr))
	{
		if (.RR("debug", 0))
			cat(g_rpt$matr[[i]]$id, "\n");

		if ( g_rpt$matr[[i]]$id=="H" )					
		{
			elm_h = g_rpt$matr[[i]];
			...
			if ( page.pos + size[2] > page.height)
			{
				...
				g_rpt$layout[[ nlay ]] <<- list(id="BR", size=c(page.width, page.break), pos=page.pos, page=page.no );
				g_rpt$pages[[page.no]] <<- list(LayoutRange = c( last.lay, nlay ) );
				page.pos <- 0;
				nlay     <- nlay+1;
				last.lay <- nlay;
				page.no  <- page.no + 1;
				...
				g_rpt$layout[[ nlay ]] <<- list(id="BR", size=c(page.width, 30), pos=page.pos, page=page.no );
			...
			g_rpt$layout[[ nlay ]] <<- list(id="H", size=size, pos=page.pos, page=page.no, str=paste(elm_h$heading.no, elm_h$str, sep=" "), cex=Report.get_cex(elm_h$level) );
			page.pos <- page.pos +  size[2];
			page.figy <- page.pos;
			nlay <- nlay+1;
		}

		if ( g_rpt$matr[[i]]$id=="P" )					
		{
			elm_p = g_rpt$matr[[i]];
			offset = g_rpt$matr[[i]]$offset;
			...
			if ( page.pos + g_rpt$line.height > page.height)
			{
				page.break<- page.height - page.pos;
				g_rpt$layout[[ nlay ]] <<- list(id="BR", size=c(page.width,page.break), pos=page.pos, page=page.no );
				g_rpt$pages[[page.no]]<<-  list(LayoutRange = c( last.lay, nlay ) );
			...
			g_rpt$layout[[nlay]] <<- list(id="P", size=size, pos=page.pos, page=page.no, offset=offset, sublay=list())
			sublay <- list();
			for (i in 1:length(p_str))
			{
				if (i==1)
			...
			g_rpt$layout[[nlay]]$sublay <<- sublay;
			page.pos <- page.pos +  size[2];
			page.figy <- page.pos;
			nlay <- nlay+1;
		}

		if ( g_rpt$matr[[i]]$id=="TB" )					
		{
			elm_tb = g_rpt$matr[[i]];
			...
			g_rpt$layout[[nlay]] <<- list(id="P", size=size, pos=page.pos, page=page.no, sublay=list(), offset=0 );
			sublay <- list(str=elm_tb$title, tab.pos=c(0.25*254), tab.align="L" );
			g_rpt$layout[[nlay]]$sublay[[1]] <<- sublay;
			page.pos <- page.pos +  size[2];
			nlay<-nlay+1;
			...
			g_rpt$layout[[nlay]] <<- list(id="TH", size=size, pos=page.pos, page=page.no, 
			...
			dat.nrow <- dim(g_rpt$matr[[i]]$data)[1];
			if (g_rpt$matr[[i]]$max.show!=0 && dat.nrow > g_rpt$matr[[i]]$max.show)
				dat.nrow <- g_rpt$matr[[i]]$max.show;
			...
			for (d in 1:dat.nrow)
			{
				...
				if (d==dat.nrow && dat.nrow<dim(g_rpt$matr[[i]]$data)[1])
					for(dr in 1:length(dat.vec) )
						dat.vec[dr] <- "...";

				g_rpt$layout[[nlay]] <<- list(id="TR", size=c(page.width, 48), pos=page.pos, page=page.no, 
						data      = dat.vec, 
						col.width = elm_tb$col.width, 
						col.align = elm_tb$col.align, 
						offset.x  = elm_tb$offset.x,
		...
		if ( g_rpt$matr[[i]]$id=="F")					
		{
			...
			if (page.figx >0)
			{
				if (page.figx + elm_f$left.margin + elm_f$size[1] <= page.width )
				{
					...
					{
						g_rpt$layout[[ nlay ]] <<- list(id="BRECT", 
								page=page.no, 
								page.figx = page.figx, 
								page.figy = page.figy,
					...
					g_rpt$layout[[ nlay ]] <<- list(id="F", page=page.no, 
		...
		if ( g_rpt$matr[[i]]$id=="NEWLINE" )					
		{
			...
			if ( page.pos + g_rpt$matr[[i]]$height > page.height)
			{
				page.break<- page.height - page.pos;
				g_rpt$layout[[ nlay ]] <<- list(id="BR", size=c(page.width,page.break), pos=page.pos, page=page.no );
				g_rpt$pages[[page.no]]<<- list(LayoutRange = c( last.lay, nlay ) );
			...
			{
				g_rpt$layout[[ nlay ]] <<- list(id="BR", size=c(page.width,g_rpt$matr[[i]]$height), pos=page.pos, page=page.no );
				page.pos <- page.pos +  g_rpt$matr[[i]]$height;
				nlay <- nlay+1;
				page.figx <- 0;
				page.figy <- page.pos;
			}
	...
	for (i in 1:length(g_rpt$pages))
	{
		...
		layHeight <- c();
		layWidth  <- c();
		lay.range <- g_rpt$pages[[i]]$LayoutRange;

		cat("The layout for page", i, " is being generated.............\n");

		laymat <- list( mat=c(1), width=c(g_rpt$page.width), height=c(0.25*254), col=1, row=1, elm_no=1, rows=list(c(1)));

		for (j in lay.range[1]:lay.range[2])
		{
			if (g_rpt$layout[[j]]$id != "F" && g_rpt$layout[[j]]$id != "BRECT")
				laymat <- laymat.add( laymat, g_rpt$layout[[j]]$id,  g_rpt$layout[[j]]$size[1], g_rpt$layout[[j]]$size[2] )
			else
				laymat <- laymat.add( laymat, g_rpt$layout[[j]]$id, 
									  g_rpt$layout[[j]]$size[1], 
									  g_rpt$layout[[j]]$size[2], 
									  page.figx = g_rpt$layout[[j]]$page.figx, 
									  page.figy = g_rpt$layout[[j]]$page.figy );
		}
		
		laymat <- laymat.add( laymat, "BR", g_rpt$page.width, 0.25*254, done=TRUE );

		g_rpt$pages[[i]]$LayoutMatrix <<- laymat$mat;
		g_rpt$pages[[i]]$LayoutWidth  <<- laymat$mat.width;
		g_rpt$pages[[i]]$LayoutHeight <<- laymat$mat.height;
	}
